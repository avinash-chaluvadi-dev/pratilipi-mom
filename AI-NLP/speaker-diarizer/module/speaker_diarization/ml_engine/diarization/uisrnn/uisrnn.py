...
class UISRNN:
    ...
    def _get_optimizer(self, optimizer, learning_rate):
        ...
        if self.estimate_sigma2:  # train sigma2
            params.append({"params": self.sigma2})  # variance parameters
    ...
    def load(self, filepath):
        ...
        self.sigma2 = nn.Parameter(torch.from_numpy(var_dict["sigma2"]).to(self.device))

        self.logger.print(
            3,
            "Loaded model with transition_bias={}, crp_alpha={}, sigma2={}, "
    ...
    def fit_concatenated(self, train_sequence, train_cluster_id, args):
        ...
        """
        # check type
        if not isinstance(train_sequence, np.ndarray) or train_sequence.dtype != float:
        ...
        if train_total_length != len(train_cluster_id):
            raise ValueError(
                "train_sequence length is not equal to " "train_cluster_id length."
            )
        ...
        for num_iter in range(args.train_iteration):
            ...
            if args.learning_rate_half_life > 0:
                if num_iter > 0 and num_iter % args.learning_rate_half_life == 0:
                    ...
                    self.logger.print(
                        2,
                        "Changing learning rate to: {}".format(
            ...
            # Sigma2 prior part.
            weight = (
                ((rnn_truth != 0).float() * mean[:-1, :, :] - rnn_truth) ** 2
            ...
            if np.remainder(num_iter, 10) == 0 or num_iter == args.train_iteration - 1:
                self.logger.print(
                    2,
                    "Iter: {:d}  \t"
                    "Training Loss: {:.4f}    \n"
    ...
    def fit(self, train_sequences, train_cluster_ids, args):
        ...
        if isinstance(train_sequences, np.ndarray):
            ...
            ) = utils.concatenate_training_data(
                [train_sequences],
                train_cluster_ids,
        ...
        for sub_idx, cluster in enumerate(cluster_seq):
            if cluster > len(new_beam_state.mean_set):  # invalid trace
                ...
                if cluster == last_cluster:
                    ...
                    loss -= (
                        np.log(self.transition_bias)
                        + np.log(new_beam_state.block_counts[cluster])
                        - np.log(sum(new_beam_state.block_counts) + self.crp_alpha)
                    )
                ...
                new_beam_state.mean_set[cluster] = (
                    new_beam_state.mean_set[cluster]
                    * ((np.array(new_beam_state.trace) == cluster).sum() - 1).astype(
                        float
                    )
                    + mean.clone()
                ...
                init_input = (
                    autograd.Variable(torch.zeros(self.observation_dim))
                    .unsqueeze(0)
                    .unsqueeze(0)
                    .to(self.device)
                )
                ...
                mean, hidden = self.rnn_model(
                    look_ahead_seq[sub_idx, :].unsqueeze(0).unsqueeze(0), hidden
                )
    ...
    def predict_single(self, test_sequence, args):
        ...
        """
        # check type
        if not isinstance(test_sequence, np.ndarray) or test_sequence.dtype != float:
        ...
        test_sequence = autograd.Variable(torch.from_numpy(test_sequence).float()).to(
            self.device
        )
        # bookkeeping for beam search
        beam_set = [BeamState()]
        for num_iter in np.arange(
            0, args["TEST_ITERATION"] * test_sequence_length, args["LOOK_AHEAD"]
        ):
            ...
            for beam_rank, beam_state in enumerate(beam_set):
                ...
                score_set[beam_rank, :] = np.pad(
                    beam_score_set,
                    np.tile(
                        [[0, max_clusters - len(beam_state.mean_set)]],
            ...
            updated_beam_set = []
            for new_beam_rank in range(np.min((len(score_ranked), args["BEAM_SIZE"]))):
    ...
    def predict(self, test_sequences, args):
        ...
        if isinstance(test_sequences, list):
            return [
                self.predict_single(test_sequence, args)
