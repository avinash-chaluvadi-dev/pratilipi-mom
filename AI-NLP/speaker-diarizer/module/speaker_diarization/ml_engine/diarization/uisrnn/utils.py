...
def concatenate_training_data(
    train_sequences, train_cluster_ids, enforce_uniqueness=True, shuffle=True
):
    ...
    train_cluster_ids = [
        x.tolist() if isinstance(x, np.ndarray) else x for x in train_cluster_ids
    ]
...
def pack_sequence(sub_sequences, seq_lengths, batch_size, observation_dim, device):
    ...
    if batch_size is None:
        rnn_input = np.zeros((sorted_seq_lengths[0], num_clusters, observation_dim))
        ...
        for i in range(batch_size):
            rnn_input[1 : sorted_seq_lengths[mini_batch[i]], i, :] = sub_sequences[
                permute_index[mini_batch[i]]
            ]
        rnn_input = autograd.Variable(torch.from_numpy(rnn_input).float()).to(device)
        packed_rnn_input = torch.nn.utils.rnn.pack_padded_sequence(
...
Performance:
  ...
  """.strip().format(
    ...
    filename = "layer_{}_{}_{:.1f}_result.txt".format(
        model_args.rnn_hidden_size, model_args.rnn_depth, model_args.rnn_dropout
    )
