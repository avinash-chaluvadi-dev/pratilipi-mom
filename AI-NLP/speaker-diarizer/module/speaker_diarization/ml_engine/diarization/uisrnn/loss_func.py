...
def weighted_mse_loss(input_tensor, target_tensor, weight=1):
    ...
    return (
        torch.mean(weighted_tensor) * weight.nelement() * entry_num / non_zero_entry_num
    )
...
def sigma2_prior_loss(num_non_zero, sigma_alpha, sigma_beta, sigma2):
    ...
    """
    return (
        (2 * sigma_alpha + num_non_zero + 2) / (2 * num_non_zero) * torch.log(sigma2)
    ).sum() + (sigma_beta / (sigma2 * num_non_zero)).sum()
