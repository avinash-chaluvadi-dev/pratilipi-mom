...
from module.speech2text.utils.utils_tools import mask_nans, check_params
                    ...
                    if grad is not None:
                        if isinstance(grad, tf.IndexedSlices):
                            if model._decoder.params.get("shared_embed", False):
                                ...
                                grad = tf.convert_to_tensor(
                                    gradient_no_duplicate_indices
                                )
        ...
        if on_horovod:
            if iter_size > 1:
                ...
                for grad, var in grads_and_vars:
                    ...
                    if isinstance(grad, tf.IndexedSlices):
                        add_grads = tf.scatter_nd_add(
                            grad_accum, grad.indices, grad.values / iter_size
                        )
                    else:
                    ...
                    with tf.control_dependencies([accum_op]):
                        red_grad_updates = opt.apply_gradients(
                            post_process_gradients(
                                reduce_gradients(
                                    grads_and_vars_accum, on_horovod=True, model=model
                                ),
                                lr=lr,
...
def post_process_gradients(grads_and_vars, summaries, lr, clip_gradients, larc_params):
...
def _global_norm_with_cast(grads_and_vars):
    return tf.global_norm(
        list(map(lambda x: tf.cast(x, tf.float32), list(zip(*grads_and_vars))[0]))
    )
...
def _clip_gradients_by_norm(grads_and_vars, clip_gradients):
    ...
    clipped_gradients, _ = _clip_by_global_norm(
        gradients, clip_gradients, use_norm=_global_norm_with_cast(grads_and_vars)
    )
