...
def piecewise_constant(
    global_step, learning_rate, boundaries, decay_rates, steps_per_epoch=None
):
...
def transformer_policy(
    ...
    decay = (
        coefficient
        * d_model ** -0.5
        * tf.minimum((step_num + 1) * ws ** -1.5, (step_num + 1) ** -0.5)
    )
...
def inv_poly_decay(
    ...
    with ops.name_scope(name, "InvDecay", [learning_rate, global_step]) as name:
        scale = (math.pow(learning_rate / min_lr, 1.0 / power) - 1.0) / decay_steps
