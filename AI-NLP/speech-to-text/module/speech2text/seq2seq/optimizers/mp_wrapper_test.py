...
from module.speech2text.seq2seq.optimizers import optimize_loss
from module.speech2text.seq2seq.optimizers.mp_wrapper import (
...
class MixedPrecisionOptimizerTests(tf.test.TestCase):
        ...
        for dtype in [tf.float16, tf.float32]:
            ...
            with tf.Graph().as_default() as g:
                ...
                with self.test_session(g, use_gpu=True) as sess:
                    ...
                    reg_loss_val, grad_val = sess.run(
                        [reg_loss, grad], {x_ph: X, y_ph: y}
                    )
            if dtype == tf.float16:
    ...
    def test_regularization_mixed(self):
        ...
        with tf.Graph().as_default() as g:
            ...
            with self.test_session(g, use_gpu=True) as sess:
                sess.run(tf.global_variables_initializer())
                reg_loss_val, grad_val = sess.run([reg_loss, grad], {x_ph: X, y_ph: y})
        ...
        for dtype in ["mixed", tf.float32]:
            with tf.Graph().as_default() as g:
                ...
                train_op = optimize_loss(
                    loss, "Adam", {}, lambda gs: fixed_lr(gs, 0.05), dtype=dtype
                )
