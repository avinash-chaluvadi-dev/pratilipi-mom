...
class BackoffScaler(object):
    def __init__(self, params):
        ...
        self.scale_max = params.get("scale_max", 2.0 ** 14)
        self.step_factor = params.get("step_factor", 2.0)
        self.step_window = params.get("step_window", 2000)

        self.iteration = tf.Variable(initial_value=0, trainable=False, dtype=tf.int64)
...
class LogMaxScaler(object):
    def __init__(self, params):
        ...
        self.x_hat = tf.Variable(initial_value=0, trainable=False, dtype=tf.float32)
    ...
    def update_op(self, has_nan, amax):
        ...
        b1_corr_assn = tf.assign(self.b1_correction, self.b1_correction * self.beta1)
        ...
        b2_corr_assn = tf.assign(self.b2_correction, self.b2_correction * self.beta2)
