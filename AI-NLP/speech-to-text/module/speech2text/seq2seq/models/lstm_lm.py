...
from module.speech2text.utils.utils_tools import deco_print, array_to_string
from module.speech2text.utils import metrics
...
class LSTMLM(EncoderDecoderModel):
    ...
    def _create_encoder(self):
        ...
        self.params["encoder_params"]["vocab_size"] = self.get_data_layer().vocab_size
        self.params["encoder_params"]["end_token"] = self.get_data_layer().end_token
        self.params["encoder_params"]["batch_size"] = self.get_data_layer().batch_size
        if not self._lm_phase:
            self.params["encoder_params"]["fc_dim"] = self.get_data_layer().num_classes
            if self.params["encoder_params"]["fc_dim"] == 2:
                self._print_f1 = True
        if self._lm_phase:
            self.params["encoder_params"]["seed_tokens"] = self.get_data_layer().params[
                "seed_tokens"
            ]
    ...
    def _create_loss(self):
        if self._lm_phase:
            self.params["loss_params"]["batch_size"] = self.get_data_layer().batch_size
            self.params["loss_params"][
                "tgt_vocab_size"
    ...
    def finalize_evaluation(self, results_per_batch, training_step=None):
        ...
        if true_pos > 0:
            ...
            deco_print(
                "EVAL Precision: {:.4f} | Recall: {:.4f} | F1: {:.4f} | True pos: {}".format(
                    prec, rec, f1, true_pos
                ),
                offset=4,
            )
        return {}
