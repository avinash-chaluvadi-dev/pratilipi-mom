...
from module.speech2text.utils.utils_tools import deco_print, clip_last_batch
from module.speech2text.seq2seq.optimizers import optimize_loss, get_regularization_loss
from module.speech2text.utils.utils_tools import check_params
...
class Model:
    ...
    def __init__(self, params, json_data, mode="train", hvd=None):
        """Model constructor.
        ...
        * **larc_params** --- dictionary with parameters for LARC (or LARS)
        ...
        if self._mode not in ["train", "infer", "eval", "package_test"]:
        ...
        if mode == "train":
            if "max_steps" not in params and "num_epochs" not in params:
                raise ValueError(
                    "For training mode either max_steps or "
        ...
        if s_fr is not None and l_fr is not None and s_fr % l_fr != 0:
            raise ValueError(
                "print_samples_steps has to be a multiple of " "print_loss_steps."
            )
        ...
        if self._hvd:
            ...
            if "gpu_ids" in self._params:
                ...
                raise ValueError(
                    'Either "gpu_ids" or "num_gpus" has to '
        ...
        if self._mode == "train":
            if "max_steps" in self._params:
                ...
                self._steps_in_epoch = (
                    self.get_data_layer().get_size_in_samples()
    ...
    def compile(self, force_var_reuse=False, checkpoint=None):
            ...
            for gpu_cnt, gpu_id in enumerate(self._gpu_ids):
                with tf.device("/gpu:{}".format(gpu_id)), tf.variable_scope(
                    ...
                    if self.params.get("use_trt", False):
                        ...
                        (
                            loss,
                            self._outputs[gpu_cnt],
                        ) = self.build_trt_forward_pass_graph(
                            input_tensors, gpu_id=gpu_cnt, checkpoint=checkpoint
                        )
                    else:
            ...
            with tf.device("/gpu:0"), tf.variable_scope(
                ...
                if self.params.get("use_trt", False):
                    ...
                    all_loss, self._output = self.build_trt_forward_pass_graph(
                        input_tensors, gpu_id=0, checkpoint=checkpoint
                    )
                else:
                    # Build regular TF graph
        ...
        if self._mode == "train":
            if "lr_policy" not in self.params:
                ...
                if (
                    "steps_per_epoch" in func_params
                    and "steps_per_epoch" not in lr_params
                    and "num_epochs" in self.params
                ):
                    lr_params["steps_per_epoch"] = self.steps_in_epoch
                lr_policy = lambda gs: self.params["lr_policy"](
            ...
            if not self.on_horovod or self._hvd.rank() == 0:
                ...
                for var in var_list:
                    ...
                    deco_print("{}".format(var.name), offset=2)
                    deco_print(
                        "shape: {}, {}".format(var.get_shape(), var.dtype), offset=4
                    )
                    if var.get_shape():
    ...
    def build_trt_forward_pass_graph(self, input_tensors, gpu_id=0, checkpoint=None):
        ...
        with temp_graph.as_default() as tf_graph:
            with tf.Session(config=tf_config) as tf_sess:
                ...
                frozen_graph = tf.graph_util.convert_variables_to_constants(
                    tf_sess, tf_sess.graph_def, output_node_names=output_node_names
                )
        ...
        for tensor in outputs:
            if tensor.name in output_sparse_tensor_map:
                ...
                component_tensors = [
                    [x for x in outputs if x.name == name][0]
