...
from module.speech2text.seq2seq.models.encoder_decoder import EncoderDecoderModel
...
class Speech2Text(EncoderDecoderModel):
    def _create_decoder(self):
        ...
        self.params["decoder_params"]["tgt_vocab_size"] = data_layer.params[
            "tgt_vocab_size"
        ]
    ...
    def _create_loss(self):
        if self.get_data_layer().params.get("autoregressive", False):
            ...
            self.params["loss_params"]["tgt_vocab_size"] = self.get_data_layer().params[
                "tgt_vocab_size"
            ]
    ...
    def _build_forward_pass_graph(self, input_tensors, gpu_id=0):
        ...
        if not isinstance(input_tensors, dict) or "source_tensors" not in input_tensors:
            raise ValueError(
                "Input tensors should be a dict containing " '"source_tensors" key'
            )
    ...
    def evaluate(self, input_values, output_values):
        ...
        for sample_id in range(batch_size):
            ...
            if self.is_bpe:
                ...
                true_text = "".join(
                    map(self.get_data_layer().params["idx2char"].get, y[:len_y])
                )
    ...
    def finalize_inference(self, results_per_batch, output_file):
        ...
        output["Diarized Filename"] = self.get_data_layer().all_files.tolist()
        output["Transcript"] = preds.tolist()
        return output
