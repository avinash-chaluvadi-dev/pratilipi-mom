...
from module.speech2text.seq2seq.optimizers.mp_wrapper import mp_regularizer_wrapper
from module.speech2text.seq2seq.parts.rnns.utils import single_cell
from .encoder import Encoder

# from seq2seq.parts.rnns.weight_drop import WeightDropLayerNormBasicLSTMCell
...
class LMEncoder(Encoder):
    ...
    def __init__(self, params, model, name="rnn_encoder_awd", mode="train"):
        """
        ...
        self.params["encoder_last_input_keep_prob"] = self.params.get(
            "encoder_last_input_keep_prob", 1.0
        )
        self.params["encoder_last_output_keep_prob"] = self.params.get(
            "encoder_last_output_keep_prob", 1.0
        )
        self.params["encoder_emb_keep_prob"] = self.params.get(
            "encoder_emb_keep_prob", 1.0
        )
        self.params["variational_recurrent"] = self.params.get(
        ...
        self.params["input_weight_keep_prob"] = self.params.get(
            "input_weight_keep_prob", 1.0
        )
        self.params["recurrent_weight_keep_prob"] = self.params.get(
        ...
        self._num_sampled = self.params.get(
            "num_sampled", self._fc_dim
        )  # if num_sampled not defined, take full softmax
    ...
    def _encode(self, input_dict):
        ...
        fake_input = tf.zeros(shape=(1, last_output_dim), dtype=self._params["dtype"])
        ...
        if (not self._lm_phase) or self._mode == "train" or self._mode == "eval":
                ...
                if use_cudnn_rnn:
                    if self._use_cell_state:
                        encoder_outputs = tf.concat(
                            [encoder_state[0][-1], encoder_state[1][-1]], axis=1
                        )
                    else:
            ...
            if (
                self._mode == "train" and self._num_sampled < self._fc_dim
            ):  # sampled softmax
                output_dict = {
                    "weights": enc_emb_w,
                    "bias": dense_biases,
