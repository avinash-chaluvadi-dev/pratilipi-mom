...
from module.speech2text.seq2seq.parts.transformer import attention_layer
    ...
    def _encode(self, input_dict):
        ...
        if self.params.get("style_embedding_enable", False):
            ...
            with tf.variable_scope("style_encoder"):
                if (
                    self._model.get_data_layer().params.get("style_input", None)
                    == "wav"
                ):
                ...
                style_embedding = tf.tile(
                    style_embedding, [1, tf.reduce_max(text_len), 1]
                )
