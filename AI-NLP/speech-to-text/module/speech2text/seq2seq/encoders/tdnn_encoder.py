...
from module.speech2text.seq2seq.parts.cnns.conv_blocks import (
    conv_actv,
    conv_bn_actv,
    conv_ln_actv,
    conv_in_actv,
...
class TDNNEncoder(Encoder):
    ...
    def _encode(self, input_dict):
        ...
        if isinstance(self._model.get_data_layer(), Speech2TextDataLayer):
            ...
            if pad_to > 0:
                num_pad = tf.mod(
                    pad_to - tf.mod(tf.reduce_max(src_length), pad_to), pad_to
                )
        else:
            print(
                "WARNING: TDNNEncoder is currently meant to be used with the",
        ...
        if self.params.get("use_conv_mask", False):
            mask = tf.sequence_mask(
                lengths=src_length, maxlen=max_len, dtype=source_sequence.dtype
            )
        ...
        for idx_convnet in range(len(convnet_layers)):
            ...
            dropout_keep = (
                convnet_layers[idx_convnet].get("dropout_keep_prob", dropout_keep_prob)
                if training
                else 1.0
            )
            ...
            for idx_layer in range(layer_repeat):
                ...
                if self.params.get("use_conv_mask", False) and (
                    ...
                    mask = tf.sequence_mask(
                        lengths=src_length, maxlen=max_len, dtype=conv_feats.dtype
                    )
                    mask = tf.expand_dims(mask, 2)
