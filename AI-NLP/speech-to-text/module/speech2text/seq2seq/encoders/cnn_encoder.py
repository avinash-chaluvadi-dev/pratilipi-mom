...
from module.speech2text.utils.utils_tools import deco_print
from .encoder import Encoder


def build_layer(
    inputs, layer, layer_params, data_format, regularizer, training, verbose=True
):
    ...
    for reg_name in ["regularizer", "kernel_regularizer", "gamma_regularizer"]:
        if reg_name not in layer_params_cp and reg_name in signature(layer).parameters:
            layer_params_cp.update({reg_name: regularizer})

    if (
        "data_format" not in layer_params_cp
        and "data_format" in signature(layer).parameters
    ):
        layer_params_cp.update({"data_format": data_format})

    # necessary to check axis for correct batch normalization processing
    if "axis" not in layer_params_cp and "axis" in signature(layer).parameters:
        layer_params_cp.update({"axis": 1 if data_format == "channels_first" else 3})

    if "training" not in layer_params_cp and "training" in signature(layer).parameters:
    ...
    if verbose:
        if hasattr(layer, "_tf_api_names"):
            ...
            layer_name = layer
        deco_print(
            "Building layer: {}(inputs, {})".format(
                layer_name,
                ", ".join(
                    "{}={}".format(key, value) for key, value in layer_params_cp.items()
                ),
            )
        )
    return outputs
...
class CNNEncoder(Encoder):
    ...
    def _encode(self, input_dict):
        ...
        for layer, layer_params in self.params["cnn_layers"]:
            x = build_layer(
                x, layer, layer_params, data_format, regularizer, self.mode == "train"
            )
