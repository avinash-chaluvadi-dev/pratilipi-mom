    ...
    def _compute_loss(self, input_dict):
        ...
        if self._offset_target_by_one:
            ...
            target_sequence = tf.slice(
                target_sequence, begin=[0, 1], size=[-1, current_ts]
            )
        else:
            current_ts = tf.cast(
                tf.minimum(
        ...
        if self._do_mask:
            ...
            mask = tf.sequence_mask(
                lengths=tgt_lengths - 1, maxlen=current_ts, dtype=logits.dtype
            )
        else:
    ...
    def _compute_loss(self, input_dict):
        ...
        if self._do_mask:
            if tgt_lengths is None:
                raise ValueError("If you are masking loss, tgt_lengths can't be None")
            mask = tf.sequence_mask(
                lengths=tgt_lengths - 1, maxlen=current_ts, dtype=tf.float32
            )
        else:
            mask = tf.cast(tf.ones_like(target_sequence), logits.dtype)

        labels = tf.one_hot(
            indices=tf.reshape(target_sequence, shape=[-1]), depth=self._tgt_vocab_size
        )
        logits = tf.reshape(logits, shape=[-1, self._tgt_vocab_size])
        ...
        with tf.name_scope(name="loss", values=[logits, labels]):
            ...
            with tf.name_scope(name="smoothing_cross_entropy", values=[logits, labels]):
                ...
                low_confidence = (1.0 - confidence) / tf.cast(
                    self._tgt_vocab_size - 1, dtype=tf.float32
                )
...
class BasicSampledSequenceLoss(Loss):
    ...
    def _compute_loss(self, input_dict):
        ...
        if "weights" in input_dict["decoder_output"]:
            print(
                "Because 'weights' is in the input_dict, we are using sampled softmax loss."
            )
            ...
            print(
                "Because 'weights' is not in the input_dict, we are using normal softmax loss."
            )
            logits = input_dict["decoder_output"]["logits"]
