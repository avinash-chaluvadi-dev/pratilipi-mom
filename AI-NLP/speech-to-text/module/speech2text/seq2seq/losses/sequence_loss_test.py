...
from module.speech2text.seq2seq.losses.sequence_loss import CrossEntropyWithSmoothing, BasicSequenceLoss
                ...
                for _o in [True, False]:
                    for _m in [True, False]:
                        ...
                        targets = tf.placeholder(
                            dtype=tf.int32, shape=[batch_size, seq_length]
                        )
                        logits = tf.placeholder(
                            dtype=tf.float32,
                            shape=[batch_size, seq_length, tgt_vocab_size],
                        )
                        tgt_lengths = tf.placeholder(dtype=tf.int32, shape=[batch_size])
                        xentropy = CrossEntropyWithSmoothing(
                            params=loss_params, model=None
                        )
                        sparse_xentropy = BasicSequenceLoss(
                            params=loss_params, model=None
                        )
                        loss_input_dict = {
                        ...
                        with self.test_session(use_gpu=True) as sess:
                            tgts = np.random.randint(
                                tgt_vocab_size, size=(batch_size, seq_length)
                            )
                            lgts = np.random.random(
                                size=[batch_size, seq_length, tgt_vocab_size]
                            )
                            feed_dict = {
                                targets: tgts,
                                logits: lgts,
                                tgt_lengths: np.array(
                                    [seq_length - offset] * batch_size
                                ),
                            }
