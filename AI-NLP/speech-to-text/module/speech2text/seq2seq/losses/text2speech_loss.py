...
class Text2SpeechLoss(Loss):
    ...
    def _compute_loss(self, input_dict):
        ...
        decoder_predictions = tf.concat([decoder_predictions, decoder_pad], axis=1)
        post_net_predictions = tf.concat([post_net_predictions, decoder_pad], axis=1)
        ...
        if self._both:
            mag_pad = tf.zeros(
                [batch_size, max_length - tf.shape(mag_pred)[1], tf.shape(mag_pred)[2]]
            )
            mag_pred = tf.concat([mag_pred, mag_pad], axis=1)
        ...
        if self.params.get("use_mask", True):
            ...
            if self._both:
                mag_loss = loss_f(labels=mag_target, predictions=mag_pred, weights=mask)
            ...
            if self._both:
                mag_loss = loss_f(labels=mag_target, predictions=mag_pred)
            stop_token_loss = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=stop_token, logits=stop_token_predictions
            )
