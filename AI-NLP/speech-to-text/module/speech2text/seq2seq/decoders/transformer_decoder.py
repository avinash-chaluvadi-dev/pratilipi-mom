...
from module.speech2text.seq2seq.parts.transformer import utils, attention_layer, ffn_layer, beam_search
from module.speech2text.seq2seq.parts.transformer.common import (
    PrePostProcessingWrapper,
    LayerNormalization,
    Transformer_BatchNorm,
)
...
class TransformerDecoder(Decoder):
    ...
    def __init__(self, params, model, name="transformer_decoder", mode="train"):
        ...
        if self.regularizer != None:
            ...
            self.regularizer = (
                self.regularizer(self.regularizer_params["scale"])
                if self.regularizer_params["scale"] > 0.0
                else None
            )
        ...
        with tf.name_scope("decode"):
            ...
            if len(self.layers) == 0:
                for _ in range(self.params["num_hidden_layers"]):
                    ...
                    self.layers.append(
                        [
                            PrePostProcessingWrapper(
                                self_attention_layer, self.params, training
                            ),
                            PrePostProcessingWrapper(
                                enc_dec_attention_layer, self.params, training
                            ),
                            PrePostProcessingWrapper(
                                feed_forward_network, self.params, training
                            ),
                        ]
                    )
            ...
            if targets is None:
                ...
                logits = self.decode_pass(
                    targets, encoder_outputs, inputs_attention_bias
                )
                return {
                    "logits": logits,
    ...
    def decode_pass(self, targets, encoder_outputs, inputs_attention_bias):
        ...
        if self.mode == "train":
            decoder_inputs = tf.nn.dropout(
                decoder_inputs, keep_prob=1 - self.params["layer_postprocess_dropout"]
            )
    ...
    def _get_symbols_to_logits_fn(self, max_decode_length):
        ...
        def symbols_to_logits_fn(ids, i, cache):
            ...
            decoder_input += tf.cast(
                x=timing_signal[i : i + 1], dtype=decoder_input.dtype
            )
    ...
    def predict(self, encoder_outputs, encoder_decoder_attention_bias):
        ...
        cache = {
            ...
            % layer: {
                "k": tf.zeros(
                    [batch_size, 0, self.params["hidden_size"]],
        ...
        logits = self.decode_pass(
            top_decoded_ids, encoder_outputs, encoder_decoder_attention_bias
        )
        return {
            "logits": logits,
