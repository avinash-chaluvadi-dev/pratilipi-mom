...
from module.speech2text.seq2seq.parts.rnns.helper import TrainingHelper, GreedyEmbeddingHelper
...
class ListenAttendSpellDecoder(Decoder):
    ...
    def _decode(self, input_dict):
        ...
        * **tgt_lengths** --- Only during training. labels lengths
        ...
        dropout_keep_prob = (
            self.params.get("dropout_keep_prob", 1.0) if self._mode == "train" else 1.0
        )
            ...
            logits = tf.while_loop(
                ...
                lambda logits: tf.concat(
                    [
                        logits,
                        tf.fill(
                            [tf.shape(logits)[0], 1, tf.shape(logits)[2]],
