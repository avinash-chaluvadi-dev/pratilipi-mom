...
from module.speech2text.seq2seq.parts.centaur import AttentionBlock
from module.speech2text.seq2seq.parts.centaur import ConvBlock
from module.speech2text.seq2seq.parts.centaur import Prenet
from module.speech2text.seq2seq.parts.transformer import utils
from module.speech2text.seq2seq.parts.transformer.common import LayerNormalization
...
class CentaurDecoder(Decoder):
    ...
    def _decode_pass(
        ...
        with tf.variable_scope("encoder_pos_encoding"):
            pos_encoding = self._positional_encoding(
                encoder_outputs, self.params["dtype"]
            )
        ...
        for i, attention in enumerate(self.attentions):
            ...
            y = attention(
                y, encoder_outputs, enc_dec_attention_bias, positions=positions
            )
    ...
    def _train(
        self, targets, encoder_outputs, enc_dec_attention_bias, sequence_lengths
    ):
        ...
        with tf.variable_scope("alignments"):
            ...
            for index in range(len(self.attentions)):
                op = (
                    "ForwardPass/centaur_decoder/attention_block_%d/attention/attention/attention_weights"
                    % index
                )
        ...
        return self._convert_outputs(
            outputs, self.reduction_factor, self._model.params["batch_size_per_gpu"]
        )

    def _infer(self, encoder_outputs, enc_dec_attention_bias, sequence_lengths):
        if sequence_lengths is None:
            maximum_iterations = self._model.get_data_layer()._params.get(
                "duration_max", 1000
            )
        else:
        ...
        state, state_shape_invariants = self._inference_initial_state(
            encoder_outputs, enc_dec_attention_bias
        )
    ...
    def _inference_initial_state(self, encoder_outputs, encoder_decoder_attention_bias):
        ...
        with tf.variable_scope("inference_initial_state"):
            ...
            state = {
                ...
                "alignment_positions": tf.zeros(
                    [n_layers, batch_size, n_heads, 1], dtype=tf.int32
                ),
                "outputs": {
                    "spec": tf.zeros(
                        [batch_size, 0, self.n_mel * self.reduction_factor]
                    ),
                    "post_net_spec": tf.zeros(
                        [batch_size, 0, self.n_mel * self.reduction_factor]
                    ),
                    "alignments": [tf.zeros([0, 0, 0, 0, 0])],
                    "stop_token_logits": tf.zeros(
                        [batch_size, 0, 1 * self.reduction_factor]
                    ),
                    "lengths": tf.zeros([batch_size], dtype=tf.int32),
                    "mag_spec": tf.zeros(
                        [batch_size, 0, self.n_mag * self.reduction_factor]
                    ),
                },
                "encoder_outputs": encoder_outputs,
            ...
            state_shape_invariants = {
                ...
                "inputs": tf.TensorShape(
                    [None, None, n_features * self.reduction_factor]
                ),
                "finished": tf.TensorShape([None]),
                "alignment_positions": tf.TensorShape([n_layers, None, n_heads, None]),
                "outputs": {
                    "spec": tf.TensorShape(
                        [None, None, self.n_mel * self.reduction_factor]
                    ),
                    "post_net_spec": tf.TensorShape(
                        [None, None, self.n_mel * self.reduction_factor]
                    ),
                    "alignments": [
                        tf.TensorShape([None, None, None, None, None]),
                    ],
                    "stop_token_logits": tf.TensorShape(
                        [None, None, 1 * self.reduction_factor]
                    ),
    ...
    def _inference_step(self, state):
        ...
        with tf.variable_scope("inference_step"):
            ...
            next_inputs = tf.where(
                state["finished"], tf.zeros_like(next_inputs), next_inputs
            )
            ...
            lengths = tf.where(
                state["finished"], lengths, lengths + 1 * self.reduction_factor
            )
            ...
            stop_token_logits = tf.concat(
                [state["outputs"]["stop_token_logits"], stop_token_logits], axis=1
            )
            ...
            with tf.variable_scope("alignments"):
                ...
                for index in range(len(self.attentions)):
                    op = (
                        forward
                        + "/centaur_decoder/while/attention_block_%d/attention/attention/attention_weights"
                        % index
                    )
                    weights_operation = tf.get_default_graph().get_operation_by_name(op)
                    weight = weights_operation.values()[0]
            ...
            alignment_positions = tf.argmax(weights, axis=-1, output_type=tf.int32)[
                :, :, :, -1:
            ]
            state["alignment_positions"] = tf.concat(
                [state["alignment_positions"], alignment_positions], axis=-1
            )
