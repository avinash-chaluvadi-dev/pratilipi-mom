...
from module.speech2text.seq2seq.parts.tacotron.tacotron_helper import (
    TacotronHelper,
    TacotronTrainingHelper,
)
from module.speech2text.seq2seq.parts.tacotron.tacotron_decoder import TacotronDecoder
from module.speech2text.seq2seq.parts.cnns.conv_blocks import conv_bn_actv
...
class Tacotron2Decoder(Decoder):
    ...
    def _decode(self, input_dict):
        ...
        if self._both:
            ...
            if self._mode == "train":
                spec, _ = tf.split(
                    spec, [self._n_feats["mel"], self._n_feats["magnitude"]], axis=2
                )
        else:
        ...
        decoder_cells = [
            single_cell(
                ...
                dp_output_keep_prob=1.0 - self.params.get("dropout_prob", 0.1),
                training=training,
            )
        ...
        if self.params.get("enable_postnet", True):
            ...
            for i, conv_params in enumerate(self.params["postnet_conv_layers"]):
                ...
                top_layer = tf.layers.dropout(
                    top_layer, rate=1.0 - dropout_keep_prob, training=training
                )

        else:
            top_layer = tf.zeros(
                [_batch_size, maximum_iterations, outputs.rnn_output.get_shape()[-1]],
                dtype=self.params["dtype"],
            )
