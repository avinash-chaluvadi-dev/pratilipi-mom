...
from module.speech2text.seq2seq.parts.rnns.attention_wrapper import (
    BahdanauAttention,
    LuongAttention,
    AttentionWrapper,
)
from module.speech2text.seq2seq.parts.rnns.gnmt import GNMTAttentionMultiCell, gnmt_residual_fn
from module.speech2text.seq2seq.parts.rnns.rnn_beam_search_decoder import BeamSearchDecoder
...
class RNNDecoderWithAttention(Decoder):
    ...
    def __init__(self, params, model, name="rnn_decoder_with_attention", mode="train"):
    ...
    def _build_attention(self, encoder_outputs, encoder_sequence_length):
        ...
        with tf.variable_scope("AttentionMechanism"):
            ...
            if self.params["attention_type"] == "bahdanau":
                ...
                self.params["attention_type"] == "gnmt"
                or self.params["attention_type"] == "gnmt_v2"
            ):
                attention_mechanism = BahdanauAttention(
                    num_units=attention_depth,
                    memory=encoder_outputs,
    ...
    def _add_residual_wrapper(cells, start_ind=1):
        for idx, cell in enumerate(cells):
            if idx >= start_ind:
                cells[
                    idx
                ] = tf.contrib.rnn.ResidualWrapper(  # pylint: disable=no-member
                    cell,
    ...
    def _decode(self, input_dict):
        ...
        * **tgt_lengths** --- Only during training. labels lengths
        ...
        tgt_inputs = (
            input_dict["target_tensors"][0] if "target_tensors" in input_dict else None
        )
        tgt_lengths = (
            input_dict["target_tensors"][1] if "target_tensors" in input_dict else None
        )
        ...
        return {
            ...
            if not time_major
            else tf.transpose(final_outputs.rnn_output, perm=[1, 0, 2]),
...
class BeamSearchRNNDecoderWithAttention(RNNDecoderWithAttention):
    ...
    def get_optional_params():
        return dict(
            RNNDecoderWithAttention.get_optional_params(),
            **{
    ...
    def _decode(self, input_dict):
        ...
        return {
            ...
            if not time_major
            else tf.transpose(final_outputs.predicted_ids[:, :, 0], perm=[1, 0, 2]),
