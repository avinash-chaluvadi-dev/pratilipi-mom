...
class FullyConnectedTimeDecoder(Decoder):
    ...
    def __init__(
        self, params, model, name="fully_connected_time_decoder", mode="train"
    ):
...
class FullyConnectedCTCDecoder(FullyConnectedTimeDecoder):
    ...
    def get_optional_params():
        return dict(
            FullyConnectedTimeDecoder.get_optional_params(),
            **{
        ...
        * **trie_weight** (float) --- weight for prefix tree vocabulary
        ...
        if self.params["use_language_model"]:
            ...
            if not os.path.exists(os.path.abspath(lib_path)):
                raise IOError(
                    "Can't find the decoder with language model library. "
            ...
            def decode_with_lm(
                ...
                return [
                    tf.SparseTensor(decoded_ixs[0], decoded_vals[0], decoded_shapes[0])
                ]
