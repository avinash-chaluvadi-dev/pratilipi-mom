...
class SpeechUtilsTests(tf.test.TestCase):
    def test_augment_audio_signal(self):
        filename = "seq2seq/test_utils/toy_speech_data/wav_files/46gc040q.wav"
    ...
    def test_get_speech_features_from_file(self):
        dirname = "seq2seq/test_utils/toy_speech_data/wav_files/"
                ...
                for window_stride in [10e-3, 5e-3, 40e-3]:
                    for window_size in [20e-3, 30e-3]:
                        for features_type in ["spectrogram", "mfcc", "logfbank"]:
                            ...
                            length = (
                                1 + (signal.shape[0] - n_window_size) // n_window_stride
                            )
                            if length % 8 != 0:
                                length += 8 - length % 8
                            ...
                            self.assertAlmostEqual(
                                np.mean(input_features), 0.0, places=6
                            )
                            self.assertAlmostEqual(
                                np.std(input_features), 1.0, places=6
                            )
                        ...
                        with self.assertRaises(AssertionError):
                            ...
                            params["window_stride"] = window_stride
                            params["sample_freq"] = 16000
                            get_speech_features_from_file(filename, params)
    ...
    def test_get_speech_features_from_file_augmentation(self):
        ...
        input_features_augm, _ = get_speech_features_from_file(filename, params)
        # just checking that result is different with and without augmentation
        self.assertTrue(np.all(np.not_equal(input_features_clean, input_features_augm)))
