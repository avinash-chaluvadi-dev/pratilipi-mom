...
from module.speech2text.seq2seq.data.data_layer import DataLayer
from module.speech2text.seq2seq.data.utils import load_pre_existing_vocabulary
from module.speech2text.seq2seq.data.speech2text.speech_utils import get_speech_features_from_file, get_speech_features
...
class Speech2TextDataLayer(DataLayer):
    ...
    def __init__(self, params, model, num_workers, worker_id, json_data=None):
        ...
        * **sample_freq** (int) --- required for precompute_mel_basis
        """
        super(Speech2TextDataLayer, self).__init__(
            params, model, num_workers, worker_id, json_data=json_data
        )
        ...
        if json_data is None:
            if params["dataset_type"] == "json":
                for json_file in params["dataset_files"]:
                    with open(json_file, "r") as f:
                        json_data = json.load(f)
                        if self._files is None:
                            self._files = utils_tools.get_train_data(
                                json_data=json_data,
                                mode=self.params["mode"],
                            )
                        # elif self._files and self.params["mode"] != "infer":
                        #     self._files.extend(
                        #         utils_tools.get_train_data(
                        #             json_data=json_data,
                        #             mode=self.params["mode"],
                        #         )
                        #     )
                        # elif self._files is None and self.params["mode"] == "infer":
                        #     self._files = utils_tools.get_train_data(
                        #         json_data=json_data,
                        #         mode=self.params["mode"],
                        #     )
                ...
                self.all_files = np.array(self._files)
            ...
            if isinstance(json_data, list):
                for json_obj in json_data:
                    if self._files is None:
                        self._files = utils_tools.get_train_data(json_data=json_obj, mode=self.params["mode"])
                    else:
                        self._files.extend(utils_tools.get_train_data(json_data=json_obj, mode=self.params["mode"]))
            else:
                self._files = utils_tools.get_train_data(json_data=json_data, mode=self.params["mode"])
            self.all_files = np.array(self._files)
            self._files = self.split_data(self.all_files)
        ...
        mel_basis = None
        if (
            self.params.get("precompute_mel_basis", False)
            and self.params["input_type"] == "logfbank"
        ):
            num_fft = self.params.get("num_fft", None) or 2 ** math.ceil(
                math.log2(self.params["window_size"] * self.params["sample_freq"])
            )
        ...
        if "time_stretch_ratio" in self.params.get("augmentation", {}):
            ...
            self.params["augmentation"]["speed_perturbation_ratio"] = self.params[
                "augmentation"
            ]["time_stretch_ratio"]
    ...
    def build_graph(self):
        with tf.device("/cpu:0"):
            ...
            if self.params["mode"] != "infer":
                ...
                if self.params["max_duration"] > 0:
                    self._dataset = self._dataset.filter(
                        lambda x, x_len, y, y_len, duration: tf.less_equal(
                            duration, self.params["max_duration"]
                        )
                    )
                if self.params["min_duration"] > 0:
                    self._dataset = self._dataset.filter(
                        lambda x, x_len, y, y_len, duration: tf.greater_equal(
                            duration, self.params["min_duration"]
                        )
                    )
                self._dataset = self._dataset.map(
                ...
                self._dataset = self._dataset.padded_batch(
                    ...
                    padded_shapes=(
                        [None, self.params["num_audio_features"]],
                        1,
                        [None],
                        1,
                    ),
                    padding_values=(
                        tf.cast(0, self.params["dtype"]),
                        0,
                        self.target_pad_value,
                        0,
                    ),
                )
            else:
                ...
                if self.params["max_duration"] > 0:
                    self._dataset = self._dataset.filter(
                        lambda x, x_len, idx, duration: tf.less_equal(
                            duration, self.params["max_duration"]
                        )
                    )
                if self.params["min_duration"] > 0:
                ...
                self._dataset = self._dataset.map(
                    lambda x, x_len, idx, duration: [x, x_len, idx],
            ...
            self._iterator = self._dataset.prefetch(
            ...
            if pad_to > 0 and self.params.get("backend") == "librosa":
                ...
                num_pad = tf.mod(
                    pad_to - tf.mod(tf.reduce_max(x_length), pad_to), pad_to
                )
    ...
    def create_interactive_placeholders(self):
        ...
        self._x_length = tf.placeholder(
            dtype=tf.int32, shape=[self.params["batch_size"]]
        )
        self._x_id = tf.placeholder(dtype=tf.int32, shape=[self.params["batch_size"]])
    ...
    def create_feed_dict(self, model_in):
        ...
        audio = np.reshape(
            audio_arr,
            [self.params["batch_size"], -1, self.params["num_audio_features"]],
        )
    ...
    def _parse_audio_transcript_element(self, element):
        ...
        if self.params.get("syn_enable", False):
            audio_filename = audio_filename.format(
                np.random.choice(self.params["syn_subdirs"])
            )

        source, audio_duration = get_speech_features_from_file(
            audio_filename, params=self.params
        )
        return (
    ...
    def _get_audio(self, wav):
        ...
        Returns:
          ...
          sample id.
        """
        source, audio_duration = get_speech_features(wav, 16000.0, self.params)
