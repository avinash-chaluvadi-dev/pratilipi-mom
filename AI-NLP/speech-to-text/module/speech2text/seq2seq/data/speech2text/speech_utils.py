...
def load_features(path, data_format):
    ...
    if data_format == "hdf5":
        ...
        raise ValueError(
            "Invalid data format for caching: ",
            data_format,
            "!\n",
...
def get_preprocessed_data_path(filename, params):
    ...
    def fix_kv(text):
        ...
        text = str(text)
        text = (
            text.replace("speed_perturbation_ratio", "sp")
            .replace(
                "noise_level_min",
                "nlmin",
            )
    ...
    preprocessed_dir = os.path.dirname(filename).replace(
        "wav", "preprocessed-" + preprocess_id
    )
    preprocessed_path = os.path.join(
        preprocessed_dir, os.path.basename(filename).replace(".wav", "")
    )
...
def get_speech_features_from_file(filename, params):
    ...
    try:
        ...
        features, duration = load_features(
            preprocessed_data_path, data_format=cache_format
        )
        ...
        if sample_freq != params["sample_freq"]:
            raise ValueError(
                ...
                ).format(params["sample_freq"], sample_freq, filename)
            )
        ...
        preprocessed_data_path = get_preprocessed_data_path(filename, params)
        save_features(
            features, duration, preprocessed_data_path, data_format=cache_format
        )
...
def augment_audio_signal(signal_float, sample_freq, augmentation):
    ...
    if "noise_level_min" in augmentation and "noise_level_max" in augmentation:
        noise_level_db = np.random.randint(
            low=augmentation["noise_level_min"], high=augmentation["noise_level_max"]
        )
        signal_float += np.random.randn(signal_float.shape[0]) * 10.0 ** (
            noise_level_db / 20.0
        )
...
def get_speech_features_librosa(
    signal,
    sample_freq,
    num_features,
    ...
    if features_type == "spectrogram":
        ...
        assert (
            num_features <= n_window_size // 2 + 1
        ...
        features = librosa.feature.mfcc(
            sr=sample_freq, S=S, n_mfcc=num_features, n_mels=2 * num_features
        ).T
...
def get_speech_features_psf(
    ...
    if augmentation is not None:
        signal = augment_audio_signal(
            signal.astype(np.float32), sample_freq, augmentation
        )
