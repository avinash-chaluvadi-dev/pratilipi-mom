...
from module.speech2text.seq2seq.data.lm.lmutils import Corpus, IMDBCorpus, SSTCorpus
from module.speech2text.seq2seq.data.utils import pad_vocab_to_eight


# from seq2seq.data.text2text.t2t import _read_and_batch_from_files
...
class WKTDataLayer(DataLayer):
    ...
    def __init__(self, params, model, num_workers=1, worker_id=0):
        super(WKTDataLayer, self).__init__(params, model, num_workers, worker_id)

        self._processed_data_folder = self.params.get(
            "processed_data_folder", "wkt-processed_data"
        )
        ...
        self.params["seed_tokens"] = [
            self.corp.dictionary.word2idx[seed_token] for seed_token in seed_tokens
        ]
    ...
    def gen(self):
        while True:
            ...
            for i in range(n_samples):
                ...
                yield (
                    self.corp.content[begin : begin + self.bptt],
                    self.corp.content[begin + 1 : begin + self.bptt + 1],
                )
    ...
    def build_graph(self):
        ...
        if self._num_workers > 1:
            _src_tgt_dataset = _src_tgt_dataset.shard(
                num_shards=self._num_workers, index=self._worker_id
            )

        if self.params["shuffle"]:
            bf_size = (
                self.get_size_in_samples()
        ...
        _src_tgt_dataset = _src_tgt_dataset.map(
            lambda x, y: ((x, tf.size(x)), (y, tf.size(y))),
...
class TextClassificationDataLayer(DataLayer):
    ...
    def __init__(self, params, model, num_workers=1, worker_id=0):
        super(TextClassificationDataLayer, self).__init__(
            params, model, num_workers, worker_id
        )
        ...
        if self._pad_lengths_to_eight and not (self._max_length % 8 == 0):
            raise ValueError(
                "If padding to 8 in data layer, then "
    ...
    def build_graph(self):
        ...
        self.batched_dataset = _src_tgt_dataset.padded_batch(
            ...
            padding_values=((self.EOS_ID, 0), (self.EOS_ID, 0)),
