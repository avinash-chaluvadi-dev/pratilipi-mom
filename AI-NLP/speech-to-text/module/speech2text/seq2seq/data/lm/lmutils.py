...
class Corpus(object):
    def __init__(self, raw_path, proc_path, change_contraction=True, limit=3):
        ...
        if not exists:
            ...
            if not raw_path:
                raise ValueError(
                    "data_root [directory to the original data] must be specified"
                )
    ...
    def _change_names(self, raw_path):
        if os.path.isfile(os.path.join(raw_path, "wiki.train.raw")):
            os.rename(
                os.path.join(raw_path, "wiki.train.raw"),
                os.path.join(raw_path, "train.txt"),
            )
            os.rename(
                os.path.join(raw_path, "wiki.valid.raw"),
                os.path.join(raw_path, "valid.txt"),
            )
            os.rename(
                os.path.join(raw_path, "wiki.test.raw"),
                os.path.join(raw_path, "test.txt"),
            )
    ...
    def create_dictionary(self, proc_path, filename):
        ...
        with open(os.path.join(proc_path, self.vocab_link), "w") as f:
            ...
            for token_id, count in self.dictionary.counter.most_common():
                ...
                f.write(
                    "\t".join(
                        [str(idx), self.dictionary.idx2word[token_id], str(count)]
                    )
                    + "\n"
                )
                idx += 1
...
class IMDBCorpus(object):
    def __init__(
        self, raw_path, proc_path, lm_vocab_link, binary=True, get_stats=False
    ):
        exists = check_exist(proc_path)
        pathlib.Path(proc_path).mkdir(exist_ok=True)
    ...
    def txt2ids(self, mode, token_file, rating_file):
        ...
        for line in open(token_file, "r"):
            ...
            reviews.append(
                [self.dictionary.word2idx.get(token, unk_id) for token in tokens]
            )
            if self._get_stats:
                for token in tokens:
                    all_tokens += 1
        ...
        if self._get_stats:
            ...
            stat_file.write(
                "\t".join(["Min", "Max", "Mean", "Median", "STD", "Total", "<=256"])
                + "\n"
            )
            stats = [
                np.min(lengths),
            ...
            stat_file.write(
                "Total {} unseen out of {} all tokens. Probability {}.\n".format(
                    all_unseen, all_tokens, all_unseen / all_tokens
                )
            )
...
class SSTCorpus(object):
    ...
    def txt2ids(self, mode):
        ...
        rating_lines = open(
            os.path.join(self.proc_path, mode + ".rat"), "r"
        ).readlines()
        ratings = [int(line.strip()) for line in rating_lines]
        ...
        if self._get_stats:
            ...
            stat_file.write(
                "\t".join(["Min", "Max", "Mean", "Median", "STD", "Total", "<=96"])
                + "\n"
            )
            stats = [
                np.min(lengths),
