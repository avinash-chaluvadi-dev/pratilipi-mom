...
def TransferMonitoredTrainingSession(
    ...
    if (restore_all or tf.train.latest_checkpoint(checkpoint_dir)) and len(
        assign_ops
    ) == 0:
    ...
    def finalize(self):
        ...
        if self._ready_for_local_init_op is None:
            ...
            def default_ready_for_local_init_op():
                return tf.report_uninitialized_variables(tf.global_variables())
            ...
            if check_name_and_shape(true_name, var, var_to_shape_map):
                ...
                if true_name not in var_to_shape_map:
                    ...
                    if var.shape != var_to_shape_map[true_name]:
                        print(
                            (
                                "var.shape [{}] does not match var_to_shape_map[true_name]"
                                "[{}]"
