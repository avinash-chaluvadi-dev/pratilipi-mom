...
# import speech2text.utils.custom_logging
...
# logger = speech2text.utils.custom_logging.get_logger()
...
def iterate_data(model, sess, compute_loss, mode, verbose, num_steps=None):
    ...
    for worker_id in range(model.num_gpus):
        ...
        try:
            ...
            total_objects = None
            # logger.exception("WARNING: Can't compute number of objects per step, since "
        ...
        if verbose:
            if size_defined:
                ...
                if (
                    ...
                    pass
                    # logger.info("Processed {}/{} batches{}".format(
                    #     processed_batches, data_size, ending
                    # ))
                pass
            else:
                pass
                # logger.info("Processed {} batches{}".format(processed_batches, ending),          end='\r')
    ...
    if verbose:
        if step > bench_start:
            ...
            logger.debug("Not enough steps for benchmarking{}".format(ending))
...
def check_params(config, required_dict, optional_dict):
    ...
    for pm, values in required_dict.items():
        if pm not in config:
            raise ValueError("{} parameter has to be specified".format(pm))
        else:
            if values == str:
                values = string_types
            if values and isinstance(values, list) and config[pm] not in values:
                raise ValueError("{} has to be one of {}".format(pm, values))
            if (
                values
                and not isinstance(values, list)
                and not isinstance(config[pm], values)
            ):
                raise ValueError("{} has to be of type {}".format(pm, values))

    for pm, values in optional_dict.items():
        if values == str:
            values = string_types
        if pm in config:
            if values and isinstance(values, list) and config[pm] not in values:
...
def get_base_config(args):
    ...
    if args.mode not in ["train", "eval", "train_eval", "infer", "interactive_infer"]:
        raise ValueError(
...
def check_logdir(args, base_config, restore_best_checkpoint=False):
    ...
    Args:
      args (): Dictionary as returned from get_base_config()
        ...
        if args.mode == "train" or args.mode == "train_eval":
            ...
            if os.path.isdir(logdir) and os.listdir(logdir) != []:
                ...
                if restore_best_checkpoint and os.path.isdir(best_ckpt_dir):
                    logger.debug("Restoring from the best checkpoint")
                    checkpoint = tf.train.latest_checkpoint(best_ckpt_dir)
                    ckpt_dir = best_ckpt_dir
                else:
                    logger.debug("Restoring from the latest checkpoint")
                    checkpoint = tf.train.latest_checkpoint(ckpt_dir)
...
def check_base_model_logdir(base_logdir, args, restore_best_checkpoint=False):
    ...
    Args:
      args (): Dictionary as returned from get_base_config()
      base_logdir (): Dictionary as returned from get_base_config()
      restore_best_checkpoint (bool): If True, will look for ckpt_dir + /best_models
    Returns:
      checkpoint: Either None if continue-learning is not set and training, or
        the name of the checkpoint used to restore the model
...
def create_model(args, base_config, config_module, base_model, hvd, checkpoint=None):
    ...
    if args.mode == "eval" or args.mode == "train_eval":
        ...
        if hvd is None or hvd.rank() == 0:
            logger.debug("Evaluation config:")
            pprint.pprint(eval_config)
    if args.mode == "infer":
        if args.infer_output_file is None:
            raise ValueError(
                '"infer_output_file" command line parameter is '
    ...
    if args.mode == "interactive_infer":
        if "interactive_infer_params" in config_module:
            nested_update(
                infer_config, copy.deepcopy(config_module["interactive_infer_params"])
            )
        if hvd is None or hvd.rank() == 0:
            logger.debug("Inference config:")
            pprint.pprint(infer_config)
    ...
    if args.benchmark:
        logger.debug("Adjusting config for benchmarking")
        ...
        if hvd is None or hvd.rank() == 0:
            logger.debug("New benchmarking config:")
            pprint.pprint(train_config)
        args.mode = "train"
