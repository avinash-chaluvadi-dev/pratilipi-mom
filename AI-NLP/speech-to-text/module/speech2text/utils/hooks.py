...
from module.speech2text.utils.utils_tools import (
    deco_print,
                ...
                if var.dtype.base_dtype == tf.float16:
                    ops.append(
                        tf.assign(
                            var,
                            tf.cast(
                                broadcast(tf.cast(var, tf.float32), root_rank),
                                tf.float16,
                            ),
                        )
                    )
...
class PrintLossAndTimeHook(tf.train.SessionRunHook):
    ...
    def after_run(self, run_context, run_values):
        ...
        if not self._model.on_horovod or self._model.hvd.rank() == 0:
            if self._print_ppl:
                deco_print(
                    "Train loss: {:.4f} | ppl = {:.4f} | bpc = {:.4f}".format(
...
class RunEvaluationHook(tf.train.SessionRunHook):
    ...
    def __init__(self, every_steps, model, last_step=-1, print_ppl=False):
        ...
        self._eval_saver = tf.train.Saver(
            save_relative_paths=True, max_to_keep=self._model.params["num_checkpoints"]
        )
    ...
    def after_run(self, run_context, run_values):
        ...
        if not self._model.on_horovod or self._model.hvd.rank() == 0:
            if self._print_ppl:
                deco_print(
                    "Validation loss: {:.4f} | ppl = {:.4f} | bpc = {:.4f}".format(
                        total_loss, math.exp(total_loss), total_loss / math.log(2)
                    ),
                    offset=4,
                )
            else:
            ...
            if (
                self._model.params["save_checkpoint_steps"]
