...
from module.speech2text.utils.helpers import (
    get_assign_ops_and_restore_dict,
...
def create_response_dict(response):
    response_dict = {}
    for audio_chunk, transcript in zip(response.get("Diarized Filename"), response.get("Transcript")):
        response_dict[audio_chunk] = transcript
    return response_dict


def create_output_json(json_data, response):
    json_data["status"] = "success"
    json_data["model"] = "speech_to_text"
    response_dict = create_response_dict(response)
    for diarized_frame in json_data.get("response"):
        if diarized_frame.get("audio_path") in response_dict:
            diarized_frame["transcript"] = response_dict.get(diarized_frame.get("audio_path"))
    return json_data


def get_train_data(json_data, mode="infer"):
    data = []
    if mode != "infer":
        for audio_path, ground_truth in zip(json_data.get("audio_chunks"), json_data.get("ground_truth")):
            data.append([audio_path, ground_truth])
    else:
        diarized_frames = json_data.get("response")
        for diarized_frame in diarized_frames:
            data.append(diarized_frame.get("audio_path"))
    return data


def save_result(json_data, output_run, out_json):
    """
    Saves response to the OUT.JSON file in results directory.
    """
    store_path = os.path.join(output_run, out_json)
...
def iterate_data(model, sess, compute_loss, mode, verbose, num_steps=None):
    ...
    sess.run(
        [model.get_data_layer(i).iterator.initializer for i in range(model.num_gpus)]
    )
    ...
    while True:
        ...
        if verbose:
            if size_defined:
                ...
                if (
                    step == 0
                    or len(fetches_vals) == 0
                    or (data_size > 10 and processed_batches % (data_size // 10) == 0)
                ):
                    deco_print(
                ...
                deco_print(
                    "Processed {} batches{}".format(processed_batches, ending), end="\r"
                )
...
def get_base_config(args):
    ...
    Args:
      args (list): The command line arugments
    ...
    parser.add_argument(
        "--mode",
        default="train",
        help='Could be "train", "eval", ' '"train_eval" or "infer"',
    )
    parser.add_argument(
        "--func_test",
    ...
    parser.add_argument(
        "--infer_output_file", default=None, help="Path to the output of inference"
    )
    parser.add_argument(
        "--continue_learning",
    ...
    parser.add_argument(
        ...
        dest="no_dir_check",
        action="store_true",
        help="whether to check that everything is correct " "with log directory",
    )
    parser.add_argument(
        "--benchmark",
        dest="benchmark",
    ...
    parser.add_argument(
        "--bench_steps", type=int, default="20", help="max_steps for benchmarking"
    )
    parser.add_argument(
        "--bench_start",
        type=int,
    ...
    parser.add_argument(
        "--debug_port", type=int, help="run TensorFlow in debug mode on specified port"
    )
    parser.add_argument(
        "--enable_logs",
        dest="enable_logs",
    ...
    if args.mode not in [
        ...
        raise ValueError(
            ...
            "['train', 'eval', 'train_eval', 'infer', 'package_test', "
            "'interactive_infer']"
        )
    config_module = runpy.run_path("config.py", init_globals={"tf": tf})
    ...
    if base_config["use_horovod"]:
        ...
        hvd = None
    base_config["use_xla_jit"] = args.use_xla_jit or base_config.get(
        "use_xla_jit", False
    )
    ...
    return args, base_config, base_model, config_module, hvd
...
def get_calibration_config(arguments):
    ...
    Args:
      arguments (str): The command line arguments
    ...
    parser.add_argument(
        "--infer_output_file",
        default="calibration/sample.pkl",
...
def check_logdir(base_config, args=None, restore_best_checkpoint=False):
    ...
    if args is None:
        ...
        try:
            ...
            if (
                args.mode == "train"
                or args.mode == "train_eval"
                or args.func_test == "train"
            ):
                ...
                args.mode == "infer"
                or args.mode == "eval"
                or args.func_test == "eval"
                or args.func_test == "infer"
...
def check_base_model_logdir(base_logdir, args=None, restore_best_checkpoint=False):
    """A helper function that ensures the logdir is setup correctly

    Args:
      args (dict): Dictionary as returned from get_base_config()
      base_logdir (dict): Dictionary as returned from get_base_config()
      restore_best_checkpoint (bool): If True, will look for ckpt_dir + /best_models
    Returns:
      checkpoint: Either None if continue-learning is not set and training, or
        the name of the checkpoint used to restore the model
    ...
    if args is None:
        ...
        if restore_best_checkpoint and os.path.isdir(
            os.path.join(ckpt_dir, "best_models")
        ):
        ...
        if (not os.path.isdir(base_logdir)) or len(os.listdir(base_logdir)) == 0:
            raise IOError(
                "The log directory for the base model is empty or does not exist."
            )
        ...
        if args.enable_logs:
            ...
            if not os.path.isdir(ckpt_dir):
                raise IOError(
                    "There's no folder 'logs' in the base model logdir. \
...
def create_model(
    ...
    Args:
      mode: Could be train, eval or infer
      args (dict): Dictionary as returned from get_base_config()
      base_config (dict): Dictionary as returned from get_base_config()
      ...
      json_data(dict): input to model checkpoint
    ...
    if args is None:
        ...
        model = base_model(params=infer_config, mode=mode, hvd=hvd, json_data=json_data)
        ...
        if (
            ...
            if "train_params" in config_module:
                nested_update(
                    train_config, copy.deepcopy(config_module["train_params"])
                )
            if hvd is None or hvd.rank() == 0:
                deco_print("Training config:")
                pprint.pprint(train_config)
        if args.mode == "eval" or args.mode == "train_eval" or args.func_test == "eval":
