...
from module.speech2text.utils.hooks import (
    PrintSamplesHook,
    RunEvaluationHook,
...
from module.speech2text.utils.helpers import (
    TransferMonitoredTrainingSession,
    TransferScaffold,
...
def train(train_model, eval_model=None, debug_port=None, custom_hooks=None):
    ...
    sess_config = tf.compat.ConfigProto(allow_soft_placement=True)
    ...
    if debug_port:
        hooks.append(tf_debug.TensorBoardDebugHook("localhost:{}".format(debug_port)))
    ...
    if len(fetches) > 1:
        total_objects = utils_tools.collect_if_horovod(total_objects, hvd, mode="sum")

    if master_worker:
        utils_tools.deco_print("Finished training")
        if step > bench_start:
            avg_time = 1.0 * total_time / num_bench_updates
            utils_tools.deco_print("Avg time per step: {:.3f}s".format(avg_time))
            if len(fetches) > 1:
            ...
            utils_tools.deco_print("Not enough steps for benchmarking")
...
def restore_and_get_results(model, checkpoint, mode):
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        results_per_batch = utils_tools.get_results_for_epoch(
            model,
            sess,
            mode=mode,
            compute_loss=False,
...
def infer(model, checkpoint, output_file):
    ...
    if not model.on_horovod or model.hvd.rank() == 0:
        ...
        utils_tools.deco_print("Finished inference")
...
def evaluate(model, checkpoint):
    ...
    if not model.on_horovod or model.hvd.rank() == 0:
        ...
        utils_tools.deco_print("Finished evaluation")
        return eval_dict
    return None
