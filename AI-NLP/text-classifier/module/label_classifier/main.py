...
def arg_parser():
    ...
    parser.add_argument(
        "--mode",
        help="train, eval, and serve mode to run the Label Classifier engine",
        type=str,
        required=True,
    )
    parser.add_argument(
...
def main():
    ...
    if cmd_args.mode == "package_test":
        ...
        if cmd_args.mode == "train_eval":
            ...
            logger.debug("Instantiating Label Classifier Engine for training..")
            network.run(
                mode=cmd_args.mode,
                save_model=cmd_args.save_model,
                continue_learning=cmd_args.continue_learning,
            ...
            logger.debug("Instantiating Label Classifier Engine for evaluation..")
            network.run(
                mode=cmd_args.mode,
                save_model=cmd_args.save_model,
                checkpoint=checkpoint,
            ...
            logger.debug("Instantiating Label Classifier Engine for serving..")
            network.run(
                mode=cmd_args.mode,
                save_model=cmd_args.save_model,
                checkpoint=checkpoint,
