...
from .model import MarkerBackbone
...
def network(
    ...
    Function to initialise the Marker engine for using the model in train, eval or serve
        ...
        :returns: Marker Engine class which contains the
                  method train, evaluate and serve for the
                  Marker Classifier model
    ...
    if mode in ["train_eval"] or (
        ...
        train_dataset = data_loader.MarkerDataset.from_dataframe(
            dataframe=train_df, mode=mode, tokenizer=tokenizer
        )
        eval_dataset = data_loader.MarkerDataset.from_dataframe(
        ...
        if json_data is None:
            serve_dataset = data_loader.MarkerDataset.from_dataframe(
                dataframe=dataframe, mode=mode, tokenizer=tokenizer, func_test=func_test
            )
            ...
            serve_dataset = data_loader.MarkerDataset.from_json(
...
def run(**kwargs):
    """
    Pluggable engine for Marker Classifier package
        :param kwargs: arguments for Marker Classifier package
    """
    ...
    if mode == "train_eval":
        if config.params.get("train").get("dataset_type") == "csv":
            ...
            dataframe = cv.split()
            logger.info("Instance of MarkerBackbone is created")
            logger.info("Instance of MarkerEngine is created")
            for fold in range(config.NUM_FOLDS):
                ...
                if eval_accuracy >= best_accuracy:
                    ...
                    utils_tools.save_model(
                        logger=logger, model_name=config.MARKER_CLASSIFIER, model=model
                    )
        ...
        if config.params.get("serve").get("dataset_type") == "csv":
            ...
            logger.debug(model_engine.serve())


class PackageTest:
    """
    Package test for functional testing of the Marker Classifier package
        :param mode: mode of Marker classifier model (train, eval or serve)
        :param tokenizer: tokenizer from huggingface library
        :param func_test: True for functional testing of package
        :param logger: custom_logger object to log the information
        :param checkpoint: Name of the checkpoint used to restore the model
        :param save_model: True to save the model otherwise False
    ...
    def test_train_component(self):
        if config.params.get("package_test").get("dataset_type") == "csv":
            ...
            for csv_file in config.params.get("package_test").get("dataset_files"):
                ...
                dataframes.append(pd.read_csv(dataframe_path))
            dataframe = pd.concat(dataframes, ignore_index=True)
            self.logger.info("Instance of MarkerBackbone is created")
            self.logger.info("Instance of MarkerEngine is created")
            model_engine = network(
                mode=self.mode,
