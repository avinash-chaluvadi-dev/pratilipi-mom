...
class MarkerEngine:
    """
    Marker Engine Class - wrapper class for encapsulating the train, evaluate and serve function of the
    Marker Classifier.
        :param model: Transformer model for Feature extraction or Sequence Classification.
        :param data_loader: Data loader/ batcher for input data.
    ...
    def train_eval(self, fold):
        """
        Training method for Marker Classifier. Saves the model after training of model is completed
        ...
        try:
            ...
            for epoch in range(self.epochs):
                ...
                for index, batch in tqdm(enumerate(self.data_loader[0])):
                    ...
                    marker = batch["marker"].to(self.device)
                    output = self.model(
                        input_ids=input_ids,
                        attention_mask=attention_mask,
                        token_type_ids=token_type_ids,
                    )
                    loss = self.loss_function(output, marker)
                    loss.backward()
                    self.optimizer.step()
                ...
                with torch.no_grad():
                    for index, batch in tqdm(enumerate(self.data_loader[1])):
                        ...
                        attention_mask = batch["attention_mask"].to(self.device)
                        token_type_ids = batch["token_type_ids"].to(self.device)
                        marker = batch["marker"].to(self.device).to(self.device)
                        output = self.model(
                            input_ids=input_ids,
                            attention_mask=attention_mask,
                            token_type_ids=token_type_ids,
                ...
                if self.save_model and eval_accuracy >= best_eval_accuracy:
                    utils_tools.modify_specs(
                        specs={
                            "finetuned_model": config.MARKER_CLASSIFIER,
                            "best_eval_accuracy": best_eval_accuracy,
                        }
                    )
                    utils_tools.save_model(
                        logger=self.logger,
                        model_name=config.MARKER_CLASSIFIER,
                ...
                self.logger.debug(
                    f"Fold {fold + 1} Epoch: {epoch+1} {train_st} {eval_st}"
                )
    ...
    def train(self, num_epochs=None):
        ...
        try:
            ...
            if self.save_model and accuracy >= best_train_accuracy:
                ...
                utils_tools.modify_specs(specs={"model": config.MARKER_CLASSIFIER})
                utils_tools.save_model(
                    logger=self.logger,
                    model_name=config.MARKER_CLASSIFIER,
                    model=self.model,
                ...
                self.logger.info(
                    f"Train accuracy of {finetuned_model_path} is more than {config.MARKER_CLASSIFIER}"
                )
        except (RuntimeError, MemoryError, ValueError, TypeError):
    ...
    def eval(self):
        ...
        Evaluation method for Marker Classifier. Saves the model after training of model is completed
            :returns (eval_loss, eval_accuracy): Evaluation loss and Evaluation accuracy
    ...
    def serve(self):
        ...
        Serve method for Marker Classifier. Returns the predictions of input data.
            :returns response: The output JSON containing process status and details.
        ...
        try:
            ...
            self.logger.exception(
                "Exception encountered while serving the Marker Engine",
                exc_info=True,
            )
            response = {
                "marker_classifier_result": {
