...
def create_dataframe(json_data):
    ...
    :param json_data: Input Json for Marker Classifier
    :return pandas.core.frame.DataFrame: dataframe which contains text and marker columns.

    """
    text_list = json_data.get(config.params.get("json").get("text_key"))
    marker_list = json_data.get(config.params.get("json").get("marker_key"))
    return pd.DataFrame({"text": text_list, "marker": marker_list})
...
def get_specs(logger, key):
    ...
    """
    json_path = os.path.join(config.ROOT_DIR, "specs.json")
    try:
...
def save_model(
    ...
    if hasattr(model, "model"):
        ...
        torch.save(
            model.state_dict(),
            os.path.join(model_path, config.MARKER_CLASSIFIER_BIN),
        )
    else:
...
def create_output_json(status, json_data, response):
    ...
    Creates output json response
        ...
        :param response: Marker Classifier response
        :return: json response with Marker Classifier predictions
    ...
    for diarized_frame in json_data.get(config.params.get("json").get("response_key")):
        ...
        if text in response_dict:
            diarized_frame["marker"] = response_dict.get(text)[0]
