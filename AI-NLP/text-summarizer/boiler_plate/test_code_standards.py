...
def get_file_list(module):
    ...
    return [f for f in os.listdir(module) if os.path.isfile(os.path.join(module, f))]
...
def test_readme_file_for_formatting():
    ...
    for module in folder_list:
        if "README.md" in get_file_list(module):
            readme_flag = True
            file_obj = open(os.path.join(module, "README.md"), "r", encoding="utf-8")
            content = file_obj.read()
            file_obj.close()
            print(content.count("#"))
            if content.count("#") < 4:
...
def test_indentations():
    ...
    for module in folder_list:
        ...
        files_list = [
            ...
            if os.path.isfile(os.path.join(this_folder, f)) and f.endswith(".py")
        ]
