...
def sync_get(url: str, ssl_verify: str):
    """Synchronous Http GET method for Keyframe extractor API"""
    ...
    # Invokes Keyframe extractor endpoint using python requests library
    try:
        response = requests.get(url=url, verify=ssl_verify).json()
        response_dict[constants.MODULE] = constants.KEYFRAME_EXT_MODULE
        response_dict[constants.COMPONENT] = constants.KEYFRAME_EXT_COMPONENT
