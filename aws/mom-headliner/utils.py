...
def sync_get(url: str, ssl_verify: str):
    """Synchronous Http GET method for Headliner API"""
    ...
    # Invokes Headliner Endpoint using python requests library
    try:
        response = requests.get(url=url, verify=ssl_verify).json()
        response_dict[constants.MODULE] = constants.HEADLINER_MODULE
        response_dict[constants.COMPONENT] = constants.HEADLINER_COMPONENT
