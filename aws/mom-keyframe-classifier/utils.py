...
def sync_get(url: str, ssl_verify: str):
    """Synchronous Http GET method for Keyframe CLS API"""
    ...
    # Invokes Keyframe CLS Endpoint using python requests library
    try:
        response = requests.get(url=url, verify=ssl_verify).json()
        response_dict[constants.MODULE] = constants.KEYFRAME_CLS_MODULE
        response_dict[constants.COMPONENT] = constants.KEYFRAME_CLS_COMPONENT
