...
def keyframe_cls_handler(event, context):
    """Handler function for invoking Keyframe CLS endpoint running on WorkOS"""

    # Fetches masked request id from event
    masked_request_id = event.get("masked_request_id")
    logger.info(
        f"Triggering keyframe classifier job corresponding to {masked_request_id}"
    )
    try:
        ...
        if masked_request_id:
            # Fetches Keyframe CLS URL from environment variable
            keyframe_cls_url = CONFIG.get("KEYFRAME_CLS_URL")
            # Invokes sync_get function which in turn invokes keyframe cls API
            keyframe_cls_response = sync_get(
                url=f"{keyframe_cls_url}/{masked_request_id}/",
                ssl_verify=CONFIG.get("SSL_VERIFY"),
            )
            logger.info(
                f"Keyframe classifier job triggered successfully --> {keyframe_cls_response}"
            )

            # Sends a success alram to signal
            signal.alarm(0)
            # Returns keyframe classifier response to step function
            return {
                "headers": {"Content-Type": "application/json"},
                "statusCode": 200,
                "isBase64Encoded": False,
                "body": keyframe_cls_response,
            }
        ...
        logger.error(
            f"Exception occured while trying to invoke Keyframe Classifier engine"
        )
        # Sends a success alram to signal
        signal.alarm(0)
