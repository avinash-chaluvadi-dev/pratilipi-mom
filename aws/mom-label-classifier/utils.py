...
def sync_get(url: str, ssl_verify: str):
    """Synchronous Http GET method for Label CLS API"""
    ...
    # Invokes Label CLS Endpoint using python requests library
    try:
        response = requests.get(url=url, verify=ssl_verify).json()
        response_dict[constants.MODULE] = constants.LABEL_CLS_MODULE
        response_dict[constants.COMPONENT] = constants.LABEL_CLS_COMPONENT
