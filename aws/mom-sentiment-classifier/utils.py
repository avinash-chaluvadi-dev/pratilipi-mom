...
def sync_get(url: str, ssl_verify: str):
    """Synchronous Http GET method for Sentiment CLS API"""
    ...
    # Invokes Sentiment CLS Endpoint using python requests library
    try:
        response = requests.get(url=url, verify=ssl_verify).json()
        response_dict[constants.MODULE] = constants.SENTIMENT_CLS_MODULE
        response_dict[constants.COMPONENT] = constants.SENTIMENT_CLS_COMPONENT
