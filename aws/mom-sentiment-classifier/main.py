...
def sentiment_cls_handler(event, context):
    """Handler function for invoking Sentiment CLS endpoint running on WorkOS"""

    # Fetches masked request id from event
    masked_request_id = event.get("masked_request_id")
    logger.info(
        f"Triggering sentiment classifier job corresponding to {masked_request_id}"
    )
    try:
        ...
        if masked_request_id:
            # Fetches Sentiment CLS URL from environment variable
            sentiment_cls_url = CONFIG.get("SENTIMENT_CLS_URL")
            # Invokes sync_get function which in turn invokes sentiment cls API
            sentiment_cls_response = sync_get(
                url=f"{sentiment_cls_url}/{masked_request_id}/",
                ssl_verify=CONFIG.get("SSL_VERIFY"),
            )
            logger.info(
                f"Sentiment classifier job triggered successfully --> {sentiment_cls_response}"
            )

            # Sends a success alram to signal
            signal.alarm(0)
            # Returns sentiment classifier response to step function
            return {
                "headers": {"Content-Type": "application/json"},
                "statusCode": 200,
                "isBase64Encoded": False,
                "body": sentiment_cls_response,
            }
        ...
        logger.error(
            f"Exception occured while trying to invoke Sentiment Classifier engine"
        )
        # Sends a success alram to signal
        signal.alarm(0)
